...
| サブシステム         | 技術スタック                                   | 目的                        |
| -------------------- | ---------------------------------------------- | --------------------------- |
| **CMS バックエンド** | Spring Boot 3.5 + Java 17                      | コンテンツ CRUD・API 提供   |
| **CMS Web 管理画面** | React 19 + Vite 6 + Ant Design 6               | 管理者向けコンテンツ管理 UI |
...
| シナリオ               | 使用システム                  | 認証方式           | Token タイプ                             | 署名方式                     |
| ---------------------- | ----------------------------- | ------------------ | ---------------------------------------- | ---------------------------- |
| **App ログイン**       | React Native → マイページ API | WebView / e-ninsho | Access Token（1h）+ Refresh Token（24h） | RS256（マイページ API 署名） |
| **App コンテンツ取得** | React Native → CMS API        | JWT Token          | 共有 JWT（Access Token）                 | RS256（公開鍵で検証）        |
| **App → Web SSO**      | React Native → マイページ Web | Ticket（30 秒）    | UUID Ticket                              | —                            |
| **マイページ Web**     | ブラウザ → マイページ API     | Session/Cookie     | JSESSIONID                               | —                            |
| **CMS 管理画面**       | CMS Web → CMS API             | JWT Token          | Access Token（30m）+ Refresh Token（8h） | HS256（CMS API 署名）        |
...
| **UI ライブラリ**  | Ant Design            | 6          | エンタープライズ UI           |
...
| 系統               | 署名アルゴリズム | 署名者                   | 検証者                | 用途                     |
| ------------------ | ---------------- | ------------------------ | --------------------- | ------------------------ |
| **Mobile App 用**  | RS256（非対称）  | マイページ API（秘密鍵） | CMS API（公開鍵）     | App ログイン後の共有 JWT |
| **CMS 管理画面用** | HS256（対称）    | CMS API（共通鍵）        | CMS API（同一共通鍵） | CMS Web 管理画面の認証   |

- **RS256（非対称署名）**: マイページ API が秘密鍵で Access Token を署名し、CMS API は公開鍵で検証のみ行う。CMS API 側では Token を偽造できないため、署名権限が分離される
- **HS256（対称署名）**: CMS 管理画面は CMS API 単独で署名・検証を完結するため、非対称署名は不要。共通鍵は Key Vault で管理し、CMS API のみがアクセス可能

#### その他セキュリティ設定

- **パスワード暗号化**: BCrypt（10 rounds、自動ソルト生成）
- **Key Vault 管理対象**: DB 接続文字列、JWT 秘密鍵/公開鍵（RS256 用）、HS256 共通鍵（CMS 用）、Azure Storage キー、Redis パスワード
...
### Refresh Token Rotation 設計

Refresh Token の盗難対策として、**Token Rotation**（使い捨て更新）方式を採用します。
...
Client                          API Server                         DB / Redis
  ...
  │                                ├─────────────────────────────────►│
  │                                │                                  │
  │                                │ 3. RT-A を無効化（is_revoked=true）
  │                                │    新しい RT-B を発行・保存      │
  │                                ├─────────────────────────────────►│
  │                                │                                  │
  │ 4. 新 Access Token + RT-B     │                                  │
  │◄───────────────────────────────┤                                  │
  │                                │                                  │
  ※ RT-A は二度と使用不可。RT-B が次回リフレッシュ用になる
```

#### 盗難検知（Reuse Detection）

```
攻撃者が盗んだ RT-A を使用した場合:

Client（正規）              API Server               攻撃者
  │                            │                       │
  │  すでに RT-B を保持        │                       │
  │                            │  RT-A で refresh      │
  │                            │◄──────────────────────┤
  │                            │                       │
  │                            │ ⚠ RT-A は is_revoked=true
  │                            │   → 盗難と判定！
  │                            │   → family_id に紐づく
  │                            │     全 Token を即時無効化
  │                            │                       │
  │  次回リクエスト時に 401    │  401 Unauthorized     │
  │  → 再ログインが必要        │ ─────────────────────►│
```

**Token Family**: 同一ログインセッションから派生した全 Refresh Token を `family_id`（UUID）で紐付け管理します。盗難検知時には `family_id` 単位で一括無効化し、正規ユーザー・攻撃者の両方を強制ログアウトさせます。

#### サーバー側 Token 管理（2 系統）

マイページ API（Redis なし）と CMS API（Redis あり）で保存先が異なります。

| 項目                       | マイページ API（Mobile 用）             | CMS API（管理画面用）                |
| -------------------------- | --------------------------------------- | ------------------------------------ |
| **Refresh Token 保存先**   | マイページ DB `refresh_tokens` テーブル | Redis `cms:refresh:{tokenHash}`      |
| **Token 保存形式**         | SHA-256 ハッシュ（平文保存しない）      | SHA-256 ハッシュ                     |
| **Rotation**               | ✅ DB レコード更新                      | ✅ 旧キー削除 + 新キー SET           |
| **Reuse Detection**        | ✅ `family_id` + `is_revoked` で判定    | ✅ 存在しないキーでのアクセス = 盗難 |
| **期限切れクリーンアップ** | バッチ処理（1 日 1 回）                 | Redis TTL 自動削除（8 時間）         |

#### マイページ側 Token 管理テーブル

マイページ API には Redis がないため、DB テーブルで Refresh Token と一時 Token を管理します。
...
| `token_hash` | VARCHAR(64)  | Refresh Token の SHA-256 ハッシュ（平文保存不可） |
| `family_id`  | VARCHAR(36)  | Token Family 識別子（UUID、Rotation 追跡用）      |
| `is_revoked` | BOOLEAN      | 無効化フラグ（Reuse Detection 用）                |
...
CREATE INDEX idx_refresh_expires_at ON refresh_tokens (expires_at);  -- クリーンアップ用
```
...
| ------------ | ------------ | -------------------------------- |
| `id`         | BIGINT PK    | 主キー（AUTO_INCREMENT）         |
| `token`      | VARCHAR(36)  | 一時 Token 値（UUID）            |
| `user_id`    | VARCHAR(100) | 認証済みユーザー                 |
| `is_used`    | BOOLEAN      | 使用済みフラグ（ワンタイム使用） |
...
CREATE INDEX idx_temp_expires_at ON temp_tokens (expires_at);  -- クリーンアップ用
```

**定期クリーンアップ**: Spring `@Scheduled` で 1 日 1 回、`expires_at < NOW()` のレコードを一括削除。

#### CMS 側 Refresh Token 管理（Redis）

CMS 管理画面の Refresh Token は Redis で管理します。

```
キー:    cms:refresh:{SHA256(refreshToken)}
値:      { "userId": "...", "familyId": "...", "createdAt": "..." }
TTL:     8 時間（Refresh Token 有効期限と一致）
```

- **Rotation 時**: 旧キーを DEL → 新キーを SET（アトミック操作）
- **ログアウト時**: 該当キーを DEL + JWT ブラックリストに Access Token を追加
- **Reuse Detection**: 存在しないキーでのリフレッシュ要求 = 盗難の可能性 → `familyId` をキーワードに SCAN して一括 DEL
...
| JWT ブラックリスト     | `jwt:blacklist:{tokenHash}` | 24 時間 |
| CMS Refresh Token 状態 | `cms:refresh:{tokenHash}`   | 8 時間  |
...
| **Web フロントエンド** | React 19 + Vite 6 + Ant Design 6   | 高速ビルド、エンタープライズ UI             |
