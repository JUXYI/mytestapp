...
- **后台服务**：Java Spring Boot REST API（部署在 Azure App Service）
- **认证系统**：Ticket-based SSO + JWT（Access/Refresh）
- **第三方认证**：e-ninsho SDK
- **云平台**：Microsoft Azure（Front Door, App Service, Azure SQL, Blob Storage, Notification Hubs 等）
...
| 模块       | 功能描述                                                                  | 依赖                    |
| ---------- | ------------------------------------------------------------------------- | ----------------------- |
| 登录认证   | 通过 WebView / 外部浏览器完成 Web 登录，获取 Ticket 并在 App 端交换 Token | Ticket-based SSO        |
| 主页菜单   | 首页导航，展示核心功能入口                                                | 后台 API                |
| 通知菜单   | 接收并展示推送通知                                                        | Notification Hubs       |
| 有用菜单   | 常用功能快速访问                                                          | 后台 API                |
| 商品栏     | 商品展示与管理（媒体来自 Blob Storage）                                   | 后台 API + Blob Storage |
...
| 单点登录   | App -> Web 端无缝切换（Ticket-based SSO）                                 | Ticket                  |
| 推送通知   | 接收服务端推送消息                                                        | Notification Hubs       |

> 说明：移动端敏感凭据（Refresh Token 等）使用 react-native-keychain 存储；一般缓存与非敏感数据使用 AsyncStorage（或 React Query 持久化）。

### 2.2 管理工具（Admin）功能模块

| 模块     | 功能描述                             | 依赖                                 |
| -------- | ------------------------------------ | ------------------------------------ |
| 登录认证 | 管理员认证（JWT 或 HttpOnly cookie） | JWT / Cookie                         |
| 内容管理 | 创建、编辑、删除内容                 | 后台 API + 数据库                    |
| 内容创建 | 富文本编辑、媒体上传                 | Blob Storage                         |
| 推送管理 | 创建、调度、监控推送任务             | Notification Hubs                    |
| 推送服务 | 推送历史、效果统计                   | Log Analytics / Application Insights |
...
└────────┬─────────┴────────┬─────────┴──────────────┬───────────────┘
   ...
   └────┬────┘         └────┬────┘         └────────┘
        ...
        │           │            │
   ┌────▼────┐  ┌────▼────┐  ┌────▼──────┐
   │ Azure   │  │ Blob    │  │ Notification│
   │ SQL DB  │  │Storage  │  │ Hubs        │
   │(Primary)│  │(Video)  │  │             │
   └─────────┘  └─────────┘  └─────────────┘
```

> 说明：架构中不使用消息队列（MQ）。异步 / 后台任务由 App Service 后台进程、Azure Functions（HTTP/Timer/Durable Functions）或 Job 调度来处理（详见第 7 节与 13 节）。
...
为 Ticket-based SSO 增加两个端点示例：
POST /api/v1/auth/ticket/exchange  — App 使用 Ticket 交换 Access/Refresh Token
POST /api/v1/auth/ticket/verify   — 验证短期 Ticket（可选）
POST /api/v1/auth/token/refresh   — 使用 Refresh Token 刷新 Access Token
POST /api/v1/auth/logout          — 撤销 Refresh Token / 登出
```
...
> 此版本采用 Ticket-based SSO（用于 App 与 Web 的单点登录），并继续使用 JWT（Access/Refresh）做 API 调用鉴权。下面补充 Ticket 方案细节。

### 5.1 Ticket-based SSO（移动端优先方案）

目标：App 使用 Web 的登录能力（或 WebView）完成认证后获得一个短期、一次性 Ticket。App 将 Ticket 发送到后端，后端校验并颁发 Access/Refresh Token 给 App。

流程：

1. 用户在 App 中选择登录 -> App 打开 WebView（或跳转浏览器）到 Web 登录页：
   - https://web.example.com/login?redirect=app
2. 用户在 Web 完成用户名/密码或 e-ninsho 等认证。
3. Web 登录成功后生成一次性 Ticket（短期、不可重放），并在重定向或通过 postMessage 将 Ticket 交给 App。
   - web -> app: window.ReactNativeWebView.postMessage(JSON.stringify({ type: 'TICKET', ticket: 'abc123' }))
   - 或重定向到深链： myapp://auth?ticket=abc123
4. App 收到 Ticket 后调用后端：
   - POST /api/v1/auth/ticket/exchange
     Body: { ticket: 'abc123', clientType: 'app' }
5. 后端验证 Ticket（与 Web 的 Session/DB/SSO 服务交互），校验通过后：
   - 颁发 Access Token（短时，示例 15 分钟）
   - 颁发 Refresh Token（更长期，示例 7 天）
   - 返回额外用户信息（必要）
6. App 存储 Token：
   - Access Token：内存（React Query/Context）并在 Axios header 中注入
   - Refresh Token：存入 Keychain
7. 以后请求通过 Authorization: Bearer {AccessToken} 访问 API，若 401 且可刷新 -> 触发刷新流程：
   - POST /api/v1/auth/token/refresh { refreshToken }
8. 登出：App 调用 /api/v1/auth/logout（后端撤销 Refresh Token / Ticket），并清除本地存储。

关键存储策略：

- Access Token：优先放内存（减少磁盘暴露），可根据业务决定短期持久化策略
- Refresh Token：使用 react-native-keychain 存储（iOS Keychain / Android Keystore），避免放入 AsyncStorage
- 其他非敏感缓存（如内容列表）可存入 React Query cache / AsyncStorage

安全建议：

- Ticket 一次性、短期（1-5 分钟）、签名防重放
- 强烈建议在生产阶段将 Refresh Token 存储在 Keychain（已按你要求调整）
- 后端维护 Refresh Token 有效列表（DB/Redis），以支持撤销

### 5.2 JWT 设计与黑名单

- Access/Refresh 机制同前文（短期 Access，长期 Refresh）
- Token 黑名单（登出）或使用 DB/Redis 存储有效 Refresh Token 列表以支持 Token 撤销
- Payload 包含 clientType、roles 等字段用于后端权限判断
...
- react-native-keychain 存储示例（伪代码见 13.2.3）
- WebView 限制 origin，使用 postMessage 并验证来源
- 生产环境优先将 Refresh Token 放到系统 Keychain / Keystore
...
## 十三、前端架构（新增章节 — 详细规则与实现建议）

本章把前端实现细节写入设计书，包含移动端（React Native）与管理工具（React）的结构、状态管理、网络层、认证（Ticket）、存储和工程实践。你已指定核心实现为：状态管理使用 React Query + AsyncStorage；网络层使用 Axios + 统一 ApiClient；认证与 SSO 使用 Ticket 方式;敏感信息使用 react-native-keychain。下面按这些约束给出可落地的设计细节与示例。

### 13.1 总体原则（适用于两端）

- TypeScript 严格模式
- 分层清晰：UI / Features / Services / State / Navigation
- 统一 Axios ApiClient（包含 Token 刷新逻辑）
- React Query 用于服务端数据
- 敏感信息（Refresh Token、长期凭证）使用 react-native-keychain 保存

---

### 13.2 移动端 — React Native + TypeScript（详细）

#### 13.2.1 推荐目录结构

- src/
  - app/ # App 入口 -> App.tsx（已移动到 src/app）
  ...
  - services/ # apiClient, authService, eNinshoService, pushService
  - hooks/ # useAuth, useTicketExchange, useNetwork
  - libs/ # react-query client config 等
  - utils/
  - i18n/
  - assets/
  - types/

说明：App 的入口文件 App.tsx 可移动到 `src/app/App.tsx`，并在根目录的 `index.js`（或 index.ts）中指向该路径（参见之前的操作步骤）。

#### 13.2.2 状态管理：React Query + AsyncStorage

- React Query 负责所有网络数据的缓存 ��� 重试与同步
- React Query 的 cache 若需持久化可用 AsyncStorage + react-query persist
- 非敏感的小量状态或持久化数据使用 AsyncStorage
- 敏感凭据（Refresh Token）使用 react-native-keychain（Keychain/Keystore）

示例：React Query 与持久化（伪代码）
...
注意：对敏感数据（如 Refresh Token）不要放入公共 cache，需要单独存储和加密（建议未来迁移到 SecureStore）。

#### 13.2.3 网络层与 ApiClient（Axios）— 使用 Keychain 获取敏感凭据

- 封装一个单例 ApiClient（Axios 实例），统一处理：
  - baseURL
  - 超时
  - headers 注入（Authorization）
  - 请求 ID（X-Request-ID）注入
  - 响应拦截器：错误分类、401 触发刷新逻辑、统一错误消息
  - 重试策略（指数回退或针对特定状态码）
- 提供轻量封装方法：get/post/put/delete、分页 helper、上传（multipart） helper。

示例：ApiClient skeleton

示例 ApiClient（关键片段，演示 Keychain 的使用方式）：
...
// 请求拦截器：注入 Access Token（从内存或 Keychain）
api.interceptors.request.use(async config => {
  // 优先从内存获取 accessToken（例如从 Auth Context）
  // 若需要读取 refreshToken 或其他敏感数据，可使用 Keychain
  // 下面示例演示读取可能存储的 accessToken（若你也用 Keychain 存 accessToken）
  try {
    const credentials = await Keychain.getGenericPassword();
    if (credentials) {
      const token = JSON.parse(credentials.password).accessToken;
      if (token) config.headers!['Authorization'] = `Bearer ${token}`;
    }
  } catch (e) {
    // 读取 Keychain 失败的处理（记录/忽略）
  }
  config.headers!['X-Request-ID'] = generateUUID();
  return config;
});

// 响应拦截器：401 -> 使用 refreshToken（从 Keychain）尝试刷新
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const resp = await axios.post(
          `${process.env.API_BASE_URL}/api/v1/auth/token/refresh`,
          { refreshToken },
        );
        // 更新 Keychain 中的凭据（将新的 access/refresh 写回）
        const updated = {
          accessToken: resp.data.accessToken,
          refreshToken: resp.data.refreshToken,
        };
        await Keychain.setGenericPassword('auth', JSON.stringify(updated));
        // 重试原请求
...
> 要点：Keychain 的读写是异步且可能抛错；Access Token 优先保存在内存以提升性能，Refresh Token 始终使用 Keychain；在某些场景 Access Token 也可写入 Keychain，但应考虑频繁读写成本。

#### 13.2.4 认证与 Ticket 交换（示例 Hook）

```ts
// src/hooks/useTicketExchange.ts
import { useMutation } from '@tanstack/react-query';
import api from '../services/apiClient';
import * as Keychain from 'react-native-keychain';

export const useTicketExchange = () => {
  return useMutation(async (ticket: string) => {
    const { data } = await api.post('/api/v1/auth/ticket/exchange', { ticket });
    // 将 accessToken & refreshToken 写入 Keychain（敏感信息）
...
#### 13.2.5 WebView 与消息安全

- WebView 接收 Ticket via postMessage 或 deep link
- 在处理 postMessage 前验证消息格式与来源（domain）
- Ticket 传给后端并写入 Keychain（见上）

#### 13.2.6 e-ninsho SDK 集成

- 把 e-ninsho 封装为 `services/eNinshoService`，供页面调用后把结果上报后端进行最终验证。
- SDK 返回 token -> App POST /api/v1/users/{userId}/verify -> 后端与 e-ninsho 验证并更新用户资料

#### 13.2.7 推送通知

- 客户端注册设备 token 到后端（POST /api/v1/devices/register）
- 后端使用 Notification Hubs 发送推送, 设备 registrationId 存储在后端 DB
- 处理前台/后台、通知点击 -> deep link 跳转到对应屏幕

#### 13.2.8 媒体与 Blob Storage

- 后端返回带 SAS 的短期 URL 或使用 CDN，App 直接下载/流式播放
- 使用 `react-native-fast-image` / `react-native-video` 做缓存和性能优化

#### 13.2.9 离线/缓存策略

- 非关键业务使用 React Query 的缓存与持久化
- 重要离线功能（如离线队列）可使用本地 DB（SQLite / MMKV）做持久队列
- React Query cache 可持久化到 AsyncStorage（非敏感）
- 离线队列若需要持久化敏感信息时注意不要将凭据写入非安全存储

#### 13.2.10 测试与发布

- 单元：Jest + React Native Testing Library
- E2E：Detox（推荐）或 Appium
- 构建/发布：Fastlane 自动化证书与上架流程；CI 使用 GitHub Actions + secret 管理

---

### 13.3 管理工具 — React + TypeScript（详要点）
...
- 使用 React Query 作为主要数据层（缓存、分页、请求状态）
- 小量 UI 状态使用 Context 或 local component state

#### 13.3.3 网络层

- 复用同样的 Axios ApiClient（web 端配置不同的 baseURL / cookie 策略）
- 若使用 Cookie-based session（Web），后端设置 HttpOnly cookie；若使用 JWT 则同移动端流程

#### 13.3.4 Authentication（Admin）

- 管理端通常直接在 Web 上登录获得 JWT (或 HttpOnly cookie)
- 前端持有最小化的 token（或使用 HttpOnly cookie）并通过 React Query 获取管理员数据与权限

#### 13.3.5 编辑器与媒体上传

- 使用成熟富文本库（tiptap / Quill / Slate）
- 媒体上传采用：前端请求后端获取 SAS -> 上传到 Blob -> 后端保存 URL

#### 13.3.6 测试与部署

- 单元：Jest + React Testing Library
- E2E：Cypress
- 部署：Azure Static Web Apps / CDN 或 App Service（若需要 SSR）

---

### 13.4 端对端类型与复用

- 建议建立一个 types 包（monorepo 或私有 npm 包）包含 DTO：User, Content, Notification 等，移动端与 Web 共用，减少类型不一致风险。
...
- Testing: Jest / RTL / Detox / Cypress
- 状态与数据: React Query + AsyncStorage（持久化与 token 存储）
- 网络: Axios + 统一 ApiClient（拦截器处理 Ticket/Token 刷新）
- Auth: Ticket-based SSO for App + JWT Access/Refresh for API
- 国际化: i18next
...
- 移动端工程骨架（RN + TS）包含：
  - App.tsx 放在 src/app
  - ApiClient（Axios）样板
  - React Query 配置 + 持久化到 AsyncStorage
  - WebView 登录 + Ticket exchange 示例（包含 Keychain 写入）
  - Token 存储与刷新示例（使用 react-native-keychain）
- Admin 工程骨架（React）包含：
  - ApiClient、React Query、Auth 守卫、内容列表页面
- Design System tokens + 示例组件
- CI YAML（GitHub Actions）与 Fastlane 基本配置
- 测试样例（Jest + RTL + Detox / Cypress skeleton）

---

## 十五、下一步建议

1. 确认是否接受当前“Ticket-based SSO”细节（例如 Ticket 生命周期、签名策略、是否需要 PKCE/CSRF 附加保护）。
2. 决定是否把 Refresh Token 从 AsyncStorage 迁移至 Secure Storage（生产建议）。
3. 我可以生成移动端工程骨架（包含 ApiClient、React Query、WebView 登录与 ticket 交换的 demo） —— 需要我现在为你生成样板代码吗？如果需要，请确认：
   - 项目名 / 包名
   - 是否需要 module-resolver 的 alias（如 @app）
   - 是否需要我把文件直接创建到你的 GitHub 仓库（请提供 owner/name 和分支）
